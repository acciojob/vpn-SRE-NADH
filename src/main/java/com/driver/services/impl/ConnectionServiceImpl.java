...
public class ConnectionServiceImpl implements ConnectionService {
    ...
    public User connect(int userId, String countryName) throws Exception{
        ...
        ServiceProvider serviceProviderWithLowest = null;
        int lowest = Integer.MAX_VALUE;
        Country country = null;
        for (ServiceProvider serviceProvider : serviceProviderList){
            List<Country> countryList = serviceProvider.getCountryList();
            for (Country country1 : countryList){
                if (countryName.equalsIgnoreCase(country1.getCountryName().toString()) && lowest>serviceProvider.getId()){
                    lowest = serviceProvider.getId();
                    serviceProviderWithLowest = serviceProvider;
                    country = country1;
                }
            }
        }

        if (serviceProviderWithLowest != null){
            Connection connection = new Connection();
            connection.setUser(user);
            connection.setServiceProvider(serviceProviderWithLowest);
            user.setMaskedIp(country.getCode() + "." + serviceProviderWithLowest.getId() + "." + userId);
            user.setConnected(true);
            user.getConnectionList().add(connection);
            serviceProviderWithLowest.getConnectionList().add(connection);
            userRepository2.save(user);
            serviceProviderRepository2.save(serviceProviderWithLowest);
        }
        else{
            throw new Exception("Unable to connect");
        }
